...
import { SyrinscapeStrategy } from "./strategies/SyrinscapeStrategy.js";
...
export class SoundHandler {
    ...
    pickSound(itemOrName, actorName, actor = null, flagType = "sound_attack") {
        ...
        if (item) {
            const flagVal = item.getFlag("ionrift-sounds", flagType);
            if (flagVal) {
    ...
    playItemSound(key, item = null, delay = 0, cooldownMs = 5000) {
        ...
        let addedDelay = 0;
        if (item) {
            const config = item.getFlag("ionrift-sounds", "sound_config");
    ...
    resolveSound(key) {
        ...
        // 1. Get Effective Bindings (Merge Layers)
            ...
            buttons: {
                save: {
                    ...
                    callback: async (html) => {
                        ...
                        if (val) {
                            await game.settings.set("ionrift-sounds", "syrinToken", val);
